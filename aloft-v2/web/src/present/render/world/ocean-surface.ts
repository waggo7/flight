...
import type { Camera, Texture, Vector4 } from 'three';
...
// Ported from v1 (src/ocean-surface.js).

/**
 * Where the water is shallow. `texture.r` is depth / 60 m (0 at the shoreline, 1 at 60 m or
 * deeper) over a square of the world; `bounds` maps world xz to its uv as
 * `uv = (xz - bounds.xy) * bounds.zw`. Outside the square the sea is deep.
 * `createIslandTerrain(...).createDepthTexture()` makes one.
 */
export interface ShallowWaterMap {
...
export interface Ocean {
  ...
  /** Re-centre the sea under the camera, in 200 m steps so the wave pattern never swims. Call every frame. */
...
/** The sea at y = 0. Shares `atmosphereUniforms`, so `uTime` moves the waves and foam. */
export function createOcean({ texture, bounds }: ShallowWaterMap): Ocean {
  const material = new ShaderMaterial({
    uniforms: {
      ...atmosphereUniforms,
      uDepthMap: { value: texture },
      uDepthBounds: { value: bounds },
      uDeepColor: { value: new Color('#0a2638') },
      uShallowColor: { value: new Color('#2f9a94') },
    },
    vertexShader,
    fragmentShader,
  });
