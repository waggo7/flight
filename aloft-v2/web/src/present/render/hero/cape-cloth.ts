...
// Ported from v1 (src/cape-cloth.js).

const GRAVITY = new Vector3(0, -9.81, 0);

export interface CapeClothOptions {
  cols?: number;
  rows?: number;
  /** Width across the shoulders, metres. */
  topWidth?: number;
...
export class CapeCloth {
  ...
  readonly pred: Float32Array;
  readonly normals: Float32Array;
  /** 0 for the pinned top row, 1 for everything else. */
  readonly invMass: Float32Array;
  /** Flat rest layout, (x, y) per point. */
  readonly rest: Float32Array;
  /** Distance constraints, four numbers each: point a, point b, rest length, stiffness. */
  readonly constraints: Float32Array;
  /** Per point: the farthest it may drift from its shoulder anchor. */
  readonly tether: Float32Array;
  readonly geometry: BufferGeometry;
  /** xyz per column, relative to the hero root in world orientation. */
  readonly anchors: Float32Array;
  readonly prevAnchors: Float32Array;
  time = 0;

  private readonly positionAttribute: BufferAttribute;
  private readonly wind = new Vector3();

  constructor({ cols = 11, rows = 16, topWidth = 0.42, bottomWidth = 1.08, length = 1.5 }: CapeClothOptions = {}) {
    this.cols = cols;
    this.rows = rows;
    ...
    const link = (a: number, b: number, stiffness: number): void => {
    ...
    const { geometry, position } = this.buildGeometry();
  ...
  private buildGeometry(): { geometry: BufferGeometry; position: BufferAttribute } {
    ...
    const position = new BufferAttribute(this.pos, 3);
    position.setUsage(DynamicDrawUsage);
    geometry.setAttribute('position', position);
    geometry.setAttribute('uv', new BufferAttribute(uv, 2));
    geometry.setIndex(index);
    // Created up front (v1 let computeVertexNormals create it) so it can be marked dynamic.
    const normal = new BufferAttribute(new Float32Array(this.count * 3), 3);
  ...
  setAnchors(anchorArray: ArrayLike<number>): void {
  ...
  drape(anchorArray: ArrayLike<number>, down: Vector3Like, back: Vector3Like): void {
    this.anchors.set(anchorArray);
    ...
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        ...
        for (let axis = 0; axis < 3; axis++) {
          const downward = axis === 0 ? down.x : axis === 1 ? down.y : down.z;
          const backward = axis === 0 ? back.x : axis === 1 ? back.y : back.z;
          const side = axis === 0 ? sx : axis === 1 ? sy : sz;
          this.pos[i * 3 + axis] = anchorArray[c * 3 + axis] + downward * along * 0.98 + backward * along * 0.14 + side * flare;
        }
  ...
  /**
   * Advance the cloth by `dt` seconds. `heroVelocity` and `frameAcceleration` are the hero's, in
   * world space; `capsule` is the body, relative to the hero root.
   */
  step(dt: number, heroVelocity: Vector3Like, frameAcceleration: Vector3Like, capsule?: ClothCapsule | null): void {
    if (dt <= 0) return;
    this.time += dt;
    ...
    const wind = this.wind.copy(heroVelocity).multiplyScalar(-1);
    const airSpeed = wind.length();
  ...
  private solveConstraints(iterations: number): void {
  ...
  private collideCapsule({ a, b, radius }: ClothCapsule): void {
    const { pred } = this;
    const abx = b.x - a.x;
    const aby = b.y - a.y;
    const abz = b.z - a.z;
