...
// vapour cone, sea spray, and spark bursts.
...
class ParticlePool {
  constructor({ capacity, color, additive, gravity = 0, drag = 0 }) {
    ...
    this.material = new THREE.ShaderMaterial({
      uniforms: { uColor: { value: new THREE.Color(color) }, uScale: { value: 500 } },
...
export class SpeedEffects {
  constructor(scene) {
    ...
    this.dust = new ParticlePool({ capacity: 160, color: '#d8cfc4', additive: false, gravity: 1.5, drag: 1.8 });
    scene.add(this.streaks.mesh, this.shockwave.ring, this.shockwave.cone, this.sparks.points, this.spray.points, this.dust.points);
    this.sprayCarry = 0;
  }
  ...
  update(dt, time, { camera, flight, projectionScale }) {
    ...
    this.dust.update(dt, projectionScale);
  }
}
